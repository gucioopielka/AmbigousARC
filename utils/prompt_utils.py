...
class AmbigousARCDataset:
    def __init__(
        ...
        if items_data is None:
            ...
            self.items_data_path = ITEMS_FILE
        ...
        self.task = task
        self.seed = seed
        self.seed_mirror = seed_mirror
        self.concept_answer_n = concept_answer_n
        self.seed_random_mat = seed_random_mat
        self.n_mirror = n_mirror
        self.example_item = example_item

        # Find the best seed for the dataset
        if find_best_seed:
            self.seed = self.find_best_seed(items_data, n_iter_seed)

        # Generate random matrices for the items
        if random_mat and task == 'discrimination':
            for idx, item in enumerate(items_data):
                items_data[idx]['D_random'] = encode_array(self.random_matrix(item, seed_random_mat))

        # Create mirror items
        if n_mirror > 0:
            items_data = create_mirror_data(items_data, n_mirror, seed_mirror)
        else:
        ...
        for item in self.items_data:
            ...
            if item['id'].split('_')[1] == '1':
                self.__seed_item = generate_seed(item['id'], seed)
            ...
            if example_item:
                ...
                example_item_data = np.random.choice(other_items)

                # Create the example item
                example_prompt, _ = getattr(self, task)(example_item_data, example=True)
    ...
    def discrimination(self, item: dict, example: bool=False) -> Tuple[str, List[str]]:
        ...
        matrix = self.answer_options[np.where(idx == 0)[0][0]]  # First matrix (correct one)
    ...
    def find_best_seed(self, items_data, n_iter = 100):
        ...
        for seed in tqdm(range(1, n_iter)):
            cfg = self.get_config()
            cfg['items_data'] = items_data
            cfg['seed'] = seed
            dataset = AmbigousARCDataset(**cfg)              
        ...
        print(f"Best seed: {best_seed} || Observed: {observed_best} || p-value: {max(randomness['p_value'])}")
        return best_seed
